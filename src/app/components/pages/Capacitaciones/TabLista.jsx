// src/components/pages/capacitaciones/TabLista.jsx
...
export default function TabLista() {
  ...
  const [form, setForm] = useState({
    ...
    puesto_id: "", // ← NUEVO: opcional, asigna al puesto al guardar
  });
  ...
  async function fetchData() {
    setLoading(true);
    const [capRes, cursosRes, puestosRes] = await Promise.all([
  ...
  function openEdit(cap) {
    ...
    setForm({
      ...
      puesto_id: "", // al editar no asignamos puesto desde aquí (eso se hace en módulo Puestos)
    });
    setErrorMsg("");
    setShowModal(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      let capIdFinal = editingId;

      if (editingId) {
        const { error } = await supabase.from("capacitaciones").update(payload).eq("id", editingId);
        if (error) throw new Error("Error: " + error.message);
      } else {
        const { data, error } = await supabase.from("capacitaciones").insert(payload).select().single();
        if (error) throw new Error("Error: " + error.message);
        capIdFinal = data.id;
      }

      // ====== NUEVO: si seleccionó un puesto, asignar y replicar ======
      if (!editingId && form.puesto_id && capIdFinal) {
        const puestoId = parseInt(form.puesto_id);

        // 1. Agregar a matriz_puesto
        const { error: errMP } = await supabase.from("matriz_puesto").insert({
          puesto_id: puestoId,
          capacitacion_id: capIdFinal,
          obligatoria: true,
        });
        if (errMP) throw new Error("Cap creada pero error al asignar al puesto: " + errMP.message);

        // 2. Replicar a todos los empleados activos de ese puesto
        const { data: emps } = await supabase
          .from("empleados").select("clave")
        ...
        if (emps && emps.length > 0) {
          const inserts = emps.map(e => ({
            emp_clave: e.clave,
            capacitacion_id: capIdFinal,
            nombre_capacitacion: payload.nombre,
            codigo_capacitacion: payload.codigo,
            completado: false,
            origen: "puesto",
            puesto_id_origen: puestoId,
            activa: true,
          }));
          const { error: errCalif } = await supabase.from("calificaciones").insert(inserts);
          if (errCalif) throw new Error("Cap asignada al puesto pero error al replicar a empleados: " + errCalif.message);
        }
      }
  ...
  async function handleDelete(cap) {
    if (!confirm(`¿Eliminar la capacitación "${cap.nombre}"?\nEsto la quitará de cualquier puesto que la tenga asignada.`)) return;
      ...
      {showModal && (
        <ModalShell title={editingId ? "Editar Capacitación" : "Nueva Capacitación"} onClose={() => setShowModal(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {/* ====== NUEVO: Dirigida a puesto (solo al crear) ====== */}
            {!editingId && (
              <div style={{
                padding: 14, backgroundColor: "#faf8ff",
                borderRadius: 12, border: "1px solid #e9e5ff",
              }}>
                <Field label={<span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Briefcase size={13} color="#7c3aed" />
                  Dirigida al puesto (opcional)
                </span>}>
                  <select value={form.puesto_id}
                    onChange={(e) => setForm({ ...form, puesto_id: e.target.value })}
                    style={inputStyle}>
                    <option value="">No asignar a ningún puesto (independiente)</option>
                    {puestos.map(p => (
                ...
                {form.puesto_id && (
                  <p style={{ fontSize: 11, color: "#10b981", margin: "8px 0 0 0", lineHeight: 1.4 }}>
                    ✓ Se agregará al puesto y se asignará automáticamente a todos los empleados con ese puesto.
                  </p>
                )}
              </div>
            )}
...
const tableTd = { padding: "12px 16px", fontSize: 13, color: "#475569" };
const pillStyle = {
